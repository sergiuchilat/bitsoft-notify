...
export default class AppConfigValidator {
  ...
  private static validateRequestTimeout(requestTimeout: number) {
    ...
    if (
      !(
        requestTimeout &&
        requestTimeout >= minRequestTimeoutValue &&
        requestTimeout <= maxRequestTimeoutValue
      )
    ) {
      throw new Error(
        `Invalid app request timeout. Must be a number between ${minRequestTimeoutValue} and ${maxRequestTimeoutValue}`,
      );
    }
  }
